...
int isAsciiDigit(int x)
{
  ...
  */
  ...
  int four2highest = !(x >> 4 ^ 0x3);
  int zero2three = !!(((x & 0xF) + (~10 + 1)) & (0x10));
...
int isLessOrEqual(int x, int y)
{
  ...
  int x_sign = !!(x >> 31);
  int y_sign = !!(y >> 31);
  int OF = (x_sign ^ y_sign) & (x_sign ^ SF);
  return (SF ^ OF) | ZF;
}
//4
/* 
...
{
  // 看了半天没找到思路，网上找答案后，发现是根据 x 和 -x 的符号位来判断
  return ((x | (~x + 1)) >> 31) + 1;
}
...
{
  /*
  * 正数需要的位数等于最高位 1 所在的二进制位加上 1 (前一个 0 表示符号位)
  * 二分法的思想，一半一半的处理，逼近最高位的 1
  * 负数取反进行处理
  */
  int half;
  int sign;
...
int floatFloat2Int(unsigned uf)
{
  ...
  if (exponent <= 150)
    signicificand >>= 150 - exponent;
  else
    signicificand <<= exponent - 150;
